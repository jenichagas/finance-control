...
async def create_debt(debt:Debt):
    installments = []
    installment_value = round(debt.value / debt.installments, 2)
    
    for i in range(debt.installments):
        ref_date = debt.start_date + relativedelta(month=i)
        installments.append({
            "name": debt.name,
    ...
    new_debts = await db.debts.find({"_id": {"$in": result.inserted_ids}}).to_list(length=debt.installments)
